...
$result = mysql_query("SELECT value FROM settings WHERE name = 'encode_extension'") or die("Error while querying DB (1): " . mysql_error());
...
if (isset($argv)) {
	...
	if ($argc >= 2 && $argv[1] == 'find') {
		...
		$result = mysql_query("SELECT value FROM settings WHERE name = 'encode_command'") or die("Error while querying DB (2): " . mysql_error());
		if (mysql_num_rows($result) == 0) {
		...
		while (TRUE) {
			...
			$result = mysql_query($query) or die("Error while querying DB (3): " . mysql_error());
			if (mysql_num_rows($result) == 0) {
				break;
			}
			$row = mysql_fetch_object($result);
			...
			if (is_dir($row->path)) { // VIDEO_TS
				...
				if ($output_file == $row->path) {
					// Oops!
					if ($encode_extension == 'm4v') {
						$output_file = substr($row->path, 0, strrpos($row->path, '.')) . '.mp4';
					} else {
...
if (isset($_GET['ee'])) {
	...
	$result = mysql_query($query) or die("Error while querying DB (4): " . mysql_error());
	$idle_files = array();
	...
	$result = mysql_query($query) or die("Error while querying DB (5): " . mysql_error());
	...
	$result = mysql_query($query) or die("Error while querying DB (6): " . mysql_error());
	if (mysql_num_rows($result) == 0) {
	...
	$result = mysql_query($query) or die("Error while querying DB (7): " . mysql_error());
	if (mysql_num_rows($result) == 0) {
		die("Error: Couldn't find $real_file in the videos5.files database.");
	}
	$row = mysql_fetch_object($result);
	...
	if ($fp) {
		...
		$result = mysql_query($query) or die("Error while querying DB (8): " . mysql_error());
		if (mysql_num_rows($result) == 0) {
			die("Error: Couldn't find $real_file in the videos5.files database.");
		}
		$row = mysql_fetch_object($result);
	...
	$result = mysql_query($query) or die("Error while querying DB (9): " . mysql_error());
	if (mysql_num_rows($result) == 0) {
	...
	$result = mysql_query($query) or die("Error while querying DB (10): " . mysql_error());
	$users = array();
	while ($row = mysql_fetch_object($result)) {
		$users[] = $row;
	}
...
function get_video_extensions() {
	...
	$result = mysql_query("SELECT value FROM settings WHERE name = 'video_extensions'") or die("Error while querying DB (11): " . mysql_error());
	if ($row = mysql_fetch_object($result)) {
		$extensions = explode(",", $row->value);
	} else {
		$extensions = array('m4v', 'amp4', 'mp4', 'ts', 'mov', 'divx', 'xvid', 'vob', 'm2v', 'avi', 'mpg', 'mpeg', 'mkv', 'm2t', 'm2ts');
	}
...
function load_paths() {
	$result = mysql_query("SELECT value FROM settings WHERE name = 'paths'") or die("Error while querying DB (12): " . mysql_error());
	if ($row = mysql_fetch_object($result)) {
		$paths = explode("\n", $row->value);
...
function insert_video($real_file, $rating=null) {
	...
	$result = mysql_query($query) or die("Error while querying DB (13): " . mysql_error());
	if (mysql_num_rows($result) > 0) {
		$row = mysql_fetch_object($result);
...
function is_html5_ready($real_file, $video_codec, $audio_codec) {
	...
	// iPad only support AVC profiles Baseline, Main & High up to level 3.1, with AAC-LC audio
	$ipad = $browser 
		&& (strpos($video_codec, 'Baseline@L1') !== FALSE 
			|| strpos($video_codec, 'Baseline@L2') !== FALSE 
			|| strpos($video_codec, 'Baseline@L3.0') !== FALSE 
...
function get_encoding_progress($real_file, $queue_position) {
	...
	if (count($results) > 0 && preg_match('/ -i (.*) -o /', $results[0], $regs)) {
...
function is_video_universal($html5_ready) {
	...
	if (!isset($ignored_devices)) {
		...
		$result = mysql_query("SELECT value FROM settings WHERE name = 'ignored_devices'") or die("Error while querying DB (14): " . mysql_error());
		if (mysql_num_rows($result) > 0) {
			$row = mysql_fetch_object($result);
...
function get_rating($nfo_files, $real_file) {
	...
	while (array_search("$parent_path/", $paths) === FALSE && $i++ < 100) {
		...
		$result = mysql_query($query) or die("Error while querying DB (15): " . mysql_error());
		if (mysql_num_rows($result) > 0) {
			$row = mysql_fetch_object($result);
			if (!empty($row->rating)) {
				$rating = $row->rating;
	...
	$result = mysql_query($query) or die("Error while querying DB (16): " . mysql_error());
	if (mysql_num_rows($result) > 0) {
		$row = mysql_fetch_object($result);
		if (!empty($row->rating)) {
			$rating = $row->rating;
...
function save_rating_nfo($rating, $real_file) {
	if (is_dir($real_file)) {
		...
		$result = mysql_query($query) or die("Error while querying DB (17): " . mysql_error());
		$video_files = array();
		while ($row = mysql_fetch_object($result)) {
...
function queue_video_for_encode($real_file) {
	...
	$result = mysql_query($query) or die("Error while querying DB (18): " . mysql_error());
	if (mysql_num_rows($result) == 0) {
		$max = 0;
	} else {
		$row = mysql_fetch_object($result);
...
function get_files_for_user($current_path) {
	...
	$query .= ") ORDER BY path";
	$result = mysql_query($query) or die("Error while querying DB (19): " . mysql_error());
	while ($row = mysql_fetch_object($result)) {
